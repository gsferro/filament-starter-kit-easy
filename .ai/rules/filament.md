...
**E BulkAction — o verbo irmão, que a policy do registro não alcança.** `BulkAction` pergunta só o verbo `*Any` (`deleteAny`, `forceDeleteAny`, …) e **nunca** consulta a policy de cada registro selecionado sem `->authorizeIndividualRecords('delete')` (`vendor/filament/actions/src/Concerns/CanBeAuthorized.php:252-266`, onde a string vira `Gate::inspect($ability, $record)`). Medido na v0.21: com `RolePolicy::delete()` negando o papel do administrador da instalação para quem não é master, a **exclusão em massa apagava esse papel assim mesmo** — a guarda por registro não era consultada, e o teste de exclusão individual estava verde. Toda BulkAction de escrita declara a autorização por registro, mesmo onde a policy ainda não decide por registro: a linha não muda comportamento hoje e fecha o buraco no dia em que decidir, que é quando ele reabriria com o diff parecendo correto. Enforço automático: `tests/Kit/AderenciaAoBlueprintTest.php` varre `app/Filament/**` e fica VERMELHO em `BulkAction::make()` sem a declaração.
