...
| **Registro** (sign-up) | registration without an invitation on `/app`, manual approval and e-mail verification ([details](/en/autenticacao/registro-aberto/)) |
| **Login** | the single login page at `/login` ([details](/en/autenticacao/login-unificado/)), the four social login providers, each with its switch, allowed panels, *Client ID* and encrypted *Client Secret*, plus the login screen footer ([details](/en/autenticacao/login-social/)) |
...
(`kit 0.37.0`). It is never presented as if it were your product's: the label is precisely what
