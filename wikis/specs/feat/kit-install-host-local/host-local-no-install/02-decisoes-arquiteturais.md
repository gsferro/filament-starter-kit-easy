...
1. **Exigir `--force`** — descartada, e não por gosto: `--force` é **destrutivo por definição**.
   ...
   (`KitInstall.php:BancoSqlite::recriar:310`), e o próprio docblock classifica isso como *"inócuo no minuto
   seguinte à instalação, destrutivo depois"* (`KitInstall.php:prepararBancoSqlite():87`). Amarrar o cadastro de um DNS local a ele
...
2. **Flag nova `--host`** — descartada por inversão de default. O precedente do projeto para
   etapa opcional é a flag **negativa** de opt-out (`KitInstall.php:--no-support:46`). Mas mesmo `--no-host`
   ...
   também em `README.md:--no-support:348`, `README.en.md` e `docs/pt/comecar/instalacao-avancada.md:--no-support:182`.
3. **Reaproveitar `--custom`** — descartada para o fluxo principal. `--custom` é o caminho de
   **reconfigurar projeto já instalado** (`KitInstall.php:customizarSemBanco():162`), e o requisito fala em
...
sem TTY nenhum (`KitInstall.php:temTerminal():147`). O requisito diz **"rode"**, não "informe como rodar".
...
- **Riscos**: `Start-Process -Verb RunAs` é assíncrono e **não devolve código de retorno
  ...
  elevação (`docs/pt/comecar/dominio-local.md:ipconfig /flushdns:103`) e portanto não prova nada

### Referências

- `docs/pt/comecar/dominio-local.md`, seção "Armadilhas" — o comando de elevação, já com o `-Wait`
- `docs/pt/comecar/dominio-local.md:o flushdns engana:99` — por que o `flushdns` mente
- `KitInstall.php:Nenhum passo aborta a instalação:26` — o invariante declarado do comando
...
`oferecerEstrela()` (`KitInstall.php:$this->oferecerEstrela():123`). Mas `banner()` (`KitInstall.php:$this->banner():120`) imprime as URLs de
acesso (`{$url}/app`, `/admin`, `/infra`) a partir de **`config('app.url')`**
(`KitInstall.php:config('app.url'):442`).

### Decisão

Chamar `oferecerHostLocal()` **entre `desvincularDoSnyk()` (`KitInstall.php:$this->desvincularDoSnyk():115`) e
`banner()` (`KitInstall.php:$this->banner():120`)** — depois de
...
- `KitInstall.php:config('app.url'):442` — de onde o `banner()` tira as URLs que imprime
- `CustomizadorDaInstalacao.php:function alinharConfigEmMemoria:559` — o precedente do padrão
...
`docs/pt/comecar/dominio-local.md:/etc/hosts:28` já documenta o caminho do Unix no mesmo bloco.

### Decisão

`match (PHP_OS_FAMILY)` — reusando o precedente de `KitInstall.php:match (PHP_OS_FAMILY):591`:
...
- **Riscos**: o CT precisa rodar nos três; o projeto já usa `skip(PHP_OS_FAMILY !== 'Windows', …)`
  (`tests/Kit/BancoSqliteTest.php:PHP_OS_FAMILY !== 'Windows':76`)
...
pergunta do customizador (`CustomizadorDaInstalacao.php:Nome do projeto:137`).
...
(`CustomizadorDaInstalacao.php:function nomeDeProjetoDocker:544`), e o resultado casa com o `meu-projeto.test` que a documentação
usa nos exemplos.

### Alternativas Consideradas

1. **Ler de `$this->resumo`** (`KitInstall.php:private array $resumo:62`) — descartada: o resumo guarda pares
...
2. **Ler o `.env` de volta** — descartada: `config('app.name')` já está alinhado em memória
   (`CustomizadorDaInstalacao.php:function alinharConfigEmMemoria:559`), e reler arquivo para saber o que acabamos de escrever é
