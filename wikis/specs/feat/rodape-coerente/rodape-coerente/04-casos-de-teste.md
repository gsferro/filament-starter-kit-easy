...
|---|---|---|---|
| ~~M33~~ **coberto** | o CSS reordena visualmente as duas linhas (ex.: `flex-col-reverse`), e o recado aparece **acima** da assinatura apesar da ordem correta no DOM | **FECHADO em 2026-09-23 (QA-13/QA-26)**: `[CT-B01]` mede a ordem VISUAL por geometria, em 5 rotas. A redação anterior o dava como candidato a CT-B — o CT-B existe. |
...
| M51 — *adversarial, rodada 2* | a tela **autenticada do layout `simple`** (verificação de e-mail, confirmação de senha) fica sem assinatura, ou sem versão, ou com as duas — é a única célula em que R1 e R2 mandam **ambas** aparecer, e a guarda por rota (M37) e o escopo por classe de login (M39) divergem ali do correto em direções opostas | **sem matador**. **Tentado**: as rotas do Breezy para verificação de e-mail e confirmação de senha exigem montar um usuário não verificado e atravessar middleware próprio, o que arrasta fronteira de outra feature para dentro deste conjunto. **Decisão**: declarar, e apontar o par como o primeiro cenário a escrever se M37 ou M39 aparecerem vivos no `pest --mutate` — **e o gatilho hoje não dispara**: não há PCOV nem Xdebug nesta instalação, então o `--mutate` não roda (QA-36). Gatilho alcançável: qualquer mudança no escopo do hook ou na guarda da blade |
| ~~M35~~ **morto** | o elemento do rodapé emite utilitárias Tailwind que não existem na folha compilada — a faixa sai **sem estilo nenhum**, com HTML byte a byte correto | **FECHADO em 2026-09-23 (QA-13/QA-26)**: `[CT-B02]` mede `font-size`, `text-align` e `opacity` COMPUTADOS — os três divergem de uma vez se a folha não chegar. |
