...
description: "Invitation is the kit's default door. The second door — open sign-up, with no invitation — exists, and it ships off:"
sidebar:
  order: 2
---
...
(`/app/login?org=acme`, or `/login?org=acme` with the [single page](/en/autenticacao/login-unificado/)) and the
...
With [the single login page](/en/autenticacao/login-unificado/) on, the sign-up address is `/cadastro`
